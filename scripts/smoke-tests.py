...
agent's Responses endpoint, and asserts case-insensitive substring rules on
the response text.
...
# without a stable redirect. Bump in lockstep with new hosted-agent releases.
...
def acquire_token() -> str:
    ...
    if token:
        return token
    result = subprocess.run(  # noqa: S603 - args are all literals, no shell interpolation
        ["az", "account", "get-access-token", "--resource", DATA_PLANE_SCOPE, "--query", "accessToken", "-o", "tsv"],  # noqa: S607 - `az` resolved from PATH by design
...
def post_response(project_endpoint: str, agent_name: str, token: str, prompt: str,
    ...
    req = urllib.request.Request(  # noqa: S310 - URL derived from controlled project_endpoint input
        url,
...
def main() -> int:
    # Default catalog is the sibling smoke-tests.json so the script is still
    # runnable standalone; the bundled action always passes --tests-file.
    ...
    # against multiple agents in one invocation.
